package com.frc869.robot.code2014.lib;
...
public class CheesyVisionServer implements Runnable {
  ...
  private int upperLeftCount_ = 0, upperRightCount_ = 0, leftCount_ = 0, rightCount_ = 0, totalCount_ = 0;
  private boolean curUpperLeftStatus_ = false, curUpperRightStatus_ = false, curLeftStatus_ = false, curRightStatus_ = false;
  ...
  private void updateCounts(boolean upperLeft, boolean upperRight, boolean left, boolean right) {
    if (counting_) {
      upperLeftCount_ += upperLeft ? 1 : 0;
      upperRightCount_ += upperRight ? 1 : 0;
      leftCount_ += left ? 1 : 0;
  ...
  public void reset() {
    upperLeftCount_ = upperRightCount_ = leftCount_ = rightCount_ = totalCount_ = 0;
    curLeftStatus_ = curRightStatus_ = false;
  }
  
  public int getUpperLeftCount() {
      return upperLeftCount_;
  }
  
  public int getUpperRightCount() {
      return upperRightCount_;
  }
  ...
  public boolean getUpperLeftStatus() {
      return curUpperLeftStatus_;
  }
  
  public boolean getUpperRightStatus() {
      return curUpperRightStatus_;
  }
        ...
        while (Timer.getFPGATimestamp() < lastHeartbeat + timeout) {
          ...
          while (is.available() > 0) {
            ...
            for (int i = 0; i < read; ++i) {
              ...
              CheesyVisionServer.this.updateCounts(upperLeftStatus, upperRightStatus, leftStatus, rightStatus);
            }
